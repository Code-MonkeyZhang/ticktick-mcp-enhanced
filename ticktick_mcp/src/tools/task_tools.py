...
def register_task_tools(mcp: FastMCP):
    ...
    async def create_tasks(tasks: Union[Dict[str, Any], List[Dict[str, Any]]]) -> str:
        ...
        try:
            ...
            return format_batch_result(
                created_tasks, failed_tasks, "created", "task", single_task,
    ...
    async def update_tasks(tasks: Union[Dict[str, Any], List[Dict[str, Any]]]) -> str:
        ...
        for i, task_data in enumerate(task_list):
            ...
            field_errors = validate_required_fields(task_data, ['task_id', 'project_id'], i)
                ...
                if date_str:
                    try:
                        ...
                        if dt.tzinfo is None:
                            validation_errors.append(f"Task {i + 1}: {date_field} must include timezone offset (e.g., +08:00 or +0000)")
                    except ValueError:
                        validation_errors.append(f"Task {i + 1}: Invalid {date_field} format '{date_str}'")
        
        if validation_errors:
            return "Validation errors found:\n" + "\n".join(validation_errors)
        
        # Update tasks and collect results
        updated_tasks = []
        failed_tasks = []
        ...
        try:
            ...
            for i, task_data in enumerate(task_list):
                try:
                    ...
                    # Only apply default timezone when updating dates
                    ...
                    failed_tasks.append(f"Task {i + 1} (ID: {task_data.get('task_id', 'Unknown')}): {str(e)}")
            
            return format_batch_result(
                updated_tasks, failed_tasks, "updated", "task", single_task,
    ...
    async def complete_tasks(tasks: Union[Dict[str, str], List[Dict[str, str]]]) -> str:
        ...
        validation_errors = []
        for i, task_data in enumerate(task_list):
            validation_errors.extend(validate_required_fields(task_data, ['task_id', 'project_id'], i))
        
        if validation_errors:
            return "Validation errors found:\n" + "\n".join(validation_errors)
        
        # Complete tasks and collect results
        completed_tasks = []
        failed_tasks = []
        ...
        try:
            ...
            return format_batch_result(
                completed_tasks, failed_tasks, "completed", "task", single_task,
    ...
    async def delete_tasks(tasks: Union[Dict[str, str], List[Dict[str, str]]]) -> str:
        ...
        # Delete tasks and collect results
        deleted_tasks = []
        failed_tasks = []
        ...
        try:
            ...
            return format_batch_result(
                deleted_tasks, failed_tasks, "deleted", "task", single_task,
    ...
    async def create_subtasks(subtasks: Union[Dict[str, Any], List[Dict[str, Any]]]) -> str:
        ...
        # Create subtasks and collect results
        created_subtasks = []
        failed_subtasks = []
            ...
            for i, subtask_data in enumerate(subtask_list):
                try:
                    ...
                    result = ticktick.create_subtask(
                        ...
                        priority=normalize_priority(subtask_data.get('priority', 0)) or 0
                    )
                    ...
                    failed_subtasks.append(f"Subtask {i + 1} ('{subtask_data.get('subtask_title', 'Unknown')}'): {str(e)}")
            
            return format_batch_result(
                created_subtasks, failed_subtasks, "created", "subtask", single_subtask,
